...
| 魔法树 | 按净资产生长；果实/金果/害虫/啄木鸟（结息不结果实） |
...
Category: spend | borrow | repay | deposit | auto_grant | auto_interest
  # 借（随借随用）= 连续写入 2 条：borrow（负债+N）+ spend（资产−N），共用 correlation_id
...
| D2 | 不同交易分开记流水；「借且强制消费」= **2 笔**：`borrow` + `spend` | 已锁定 |
| D3 | **结息不触发果实** | 已锁定 |
...
|------|------|------|--------------|--------|
| 支出 | −N | — | `spend` ×1 | — |
| 借（随借随用） | −N | +N | `borrow` ×1 **且** `spend` ×1（同 correlation_id） | +害虫（按「借」一次） |
| 还 | — | −N | `repay` ×1 | 负债归零 → +啄木鸟 |
| 手动存入（家长） | +N | — | `deposit` ×1 | +果实 |
| 自动派发 | +N | — | `auto_grant` ×1 | +果实 |
...
- 果实：`deposit` / `auto_grant` 等认定的增资（**不含结息**）每次 +1；每 100 果 → 1 金果
- 害虫：每次「借」操作 +1（一次借一次害虫，即使 2 条流水）
- 啄木鸟：每次还清债务 +1
- 月果（结果阶段）：月均时间资产 = 每日结算资产余额之和 ÷ 30；按档位发果（取最高档），每月每账户至多一次：
  - ≥100 → 1 果
  - ≥200 → 4 果
  - ≥300 → 8 果
  - ≥400 → 12 果
  - ≥500 → 16 果
  - …（`n=floor(均/100)`；`n=1 → 1`；`n≥2 → 4×(n-1)`）

### 4.4 日终工作流

```
每日 00:00（固定 Asia/Shanghai / UTC+8）
  1) 分账户：资产结息流水 + 负债结息流水（均不触发果实）
  2) 若当日为月末：自动对账
  3) 备份：流水 + 资产负债表 + 每账户设置
每日 01:00（同上时区）
  分账户自动派发 → auto_grant（+果实）
