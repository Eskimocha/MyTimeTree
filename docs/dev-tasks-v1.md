...
| E2.4 | 借：原子写 2 条流水 `borrow`+`spend`（同 correlation_id）；害虫+1 次 | 余额与挂件断言 | Done |
| E2.5 | 还：预设场景/自定义；还清→啄木鸟 | | Done |
| E2.6 | 家长手动 `deposit` + `auto_grant` →果实；**结息不结果实** | 100 果→金果 | Done |
...
| E4.2 | 挂件计数器状态机 | 果/金果/虫/鸟 | Done |
| E4.3 | 月均资产分级奖励（结果阶段）：≥100→1；n≥2 时 `4×(n-1)`（200→4、300→8…） | 假日历；每月至多 1 次 | Done |

## E5 — 设置与安全

| ID | 任务 | 验收 | 状态 |
|----|------|------|------|
| E5.1 | **每账户**设置 CRUD（派发、默认支出/还入、利率、预设文案） | 切换账户后设置隔离 | Done |
